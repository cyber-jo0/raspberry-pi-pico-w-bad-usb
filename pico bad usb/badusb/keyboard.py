...
try:
    import secrets  # optional user config (LAYOUT)
except ImportError:
    secrets = None

# Choose keyboard layout dynamically; defaults to QWERTY
try:
    from . import layouts
except ImportError:
    import layouts
...
_layout_name = "QWERTY"
if secrets and hasattr(secrets, "LAYOUT"):
    _layout_name = getattr(secrets, "LAYOUT", "QWERTY").upper()

ASCII = getattr(layouts, _layout_name, layouts.QWERTY)
...
class Keyboard:
    ...
    def hotkey(self, *keycodes: int) -> None:
        self.press(*keycodes)
        sleep(0.01)  # Small delay for key registration
        self.release()
